...
def get_token(pipeline_id, module):
    with patch(
        f"aws_asdi_pipelines.pipelines.{pipeline_id}.{module}.get_token",
        return_value=token,
        autospec=True,
    ) as m:
        yield m
...
def requests(pipeline_id, module):
    with patch(
        f"aws_asdi_pipelines.pipelines.{pipeline_id}.{module}.requests", autospec=True
    ) as m:
        yield m
...
def run_query(pipeline_id):
    with patch(
        f"aws_asdi_pipelines.pipelines.{pipeline_id}.historic.run_query",
...
def boto3(pipeline_id, query_value):
    with patch(
        f"aws_asdi_pipelines.pipelines.{pipeline_id}.historic.boto3", autospec=True
    ) as m:
